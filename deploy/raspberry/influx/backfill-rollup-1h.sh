...
mean |> set(key: "_measurement", value: "sensor_rollup_1h") |> to(bucket: "${ROLLUP_BUCKET}", tagColumns: ["node_id"])
min |> set(key: "_measurement", value: "sensor_rollup_1h") |> to(bucket: "${ROLLUP_BUCKET}", tagColumns: ["node_id"])
max |> set(key: "_measurement", value: "sensor_rollup_1h") |> to(bucket: "${ROLLUP_BUCKET}", tagColumns: ["node_id"])
count |> set(key: "_measurement", value: "sensor_rollup_1h") |> to(bucket: "${ROLLUP_BUCKET}", tagColumns: ["node_id"])
FLUX
}
