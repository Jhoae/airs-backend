...
CREATE TABLE spaces (
  ...
  space_type ENUM('CLASSROOM','LAB','OFFICE','OTHER','READING_ROOM') NOT NULL,
  latitude DECIMAL(10, 7) NULL,
  longitude DECIMAL(10, 7) NULL,
  created_at DATETIME NOT NULL,
  deleted_at DATETIME NULL,
...
CREATE TABLE users (
  ...
  role ENUM('ADMIN','ROOT_ADMIN','USER') NOT NULL,
  created_at DATETIME NOT NULL,
  PRIMARY KEY (id),
...
CREATE TABLE node_status_snapshots (
  id BIGINT NOT NULL AUTO_INCREMENT,
  node_id VARCHAR(80) NOT NULL,
  connection_status ENUM('OFFLINE','ONLINE','UNKNOWN','WEAK') NOT NULL,
  sensor_status ENUM('ABNORMAL','NORMAL','NO_DATA') NOT NULL,
...
CREATE TABLE space_status_snapshots (
  ...
  co2_ppm INT NULL,
  human_detected BOOLEAN NULL,
  occupancy_status ENUM('OCCUPIED','UNKNOWN','UNOCCUPIED') NULL,
